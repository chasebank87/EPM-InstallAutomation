...
1. When choosing between version 900 and version 11.1.2.4 you need to modify the options you use. For instance RAF is no longer included in version 900. So we need to use the option -rfDB (EPMS_HFR) instead, and install switch -installRF. Disclosure is also not included so you will need to leave that install and DB switch out when running your command.
...
3. The switches -startEPM, and -validate should only be used for standalone environments
  ...
  #### -superSilentConfig
   ...
   #### -superSilentAll
       ...
       * Optional Sub Options: (Default answer to below install switches is $false)
          ...
          * -fdmDB (String | Example: EPMS_FDM)
          * -profitDB (String | Example: EPMS_PCM)
          * -startEPM ($true | $false {This should only be used when doing standalone installs})
          * -validate ($true | $false {This should only be used when doing standalone installs})
          * -distributedEssbase ($true | $false)
          * -distributedHFM ($true | $false)
          * -distributedPlanning ($true | $false)
          * -distributedFDM ($true | $false)
...
4. <a href="https://github.com/chasebank87/EPMSilent-InstallAutomation/blob/master/Help/Distributed-HFM-FDM-PLN-ESB.md">Distributed HFM, FDM, PLN, ESB __(Five Server Environment)__</a>
#### 11.1.2.4.900
...
4. <a href="https://github.com/chasebank87/EPMSilent-InstallAutomation/blob/master/Help/Distributed-HFM-FDM-PLN-ESB900.md">Distributed HFM, FDM, PLN, ESB __(Five Server Environment)__</a>
