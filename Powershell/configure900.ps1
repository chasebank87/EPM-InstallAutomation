...
#region configuration details
    ...
    #region user input for recommended configuration
        ...
        if(!$superSilentConfig -or $superSilentConfig -eq $false){
            ...
            if($frSilent -ne $null) {
                if($inputRecommended -eq 1) {
                    Write-Host "============== FR DB Name ==============="
                    Write-Host "What is your FR Database name? "
                    Write-Host ""
                    #Write-Host ""
                    Write-Host "Type Quit to Exit the Installer" -ForegroundColor Yellow
                    Write-Host "=========================================="
                    $inputFRDB = Read-Host
                    if ($inputFRDB -like "*quit*") {Exit}
...
#region set variables
    ...
    if($standalone -eq $true){
        ...
        $headerConfigureSilent = Get-Content -Path "$($installerPath)\Variables\Property Files\11.1.2.4.900\Config\Standalone\header" -Raw
        ...
        if($configFoundation -ne $false){
            ...
            $fndConfigureSilent = Get-Content -Path "$($installerPath)\Variables\Property Files\11.1.2.4.900\Config\Standalone\foundation" -Raw
        ...
        if($configEssbase -ne $false){
            ...
            $apsConfigureSilent =  Get-Content -Path "$($installerPath)\Variables\Property Files\11.1.2.4.900\Config\Standalone\aps" -Raw
        ...
        if($configEPMA -ne $false){
            ...
            $bpmaConfigureSilent = Get-Content -Path "$($installerPath)\Variables\Property Files\11.1.2.4.900\Config\Standalone\epma" -Raw
        ...
        if($configEssbase -ne $false){
            ...
            $bpmsConfigureSilent = Get-Content -Path "$($installerPath)\Variables\Property Files\11.1.2.4.900\Config\Standalone\essbaseStudio" -Raw
        ...
        if($configCALC -ne $false){
            ...
            $calcConfigureSilent = Get-Content -Path "$($installerPath)\Variables\Property Files\11.1.2.4.900\Config\Standalone\calc" -Raw
        ...
        if($configEssbase -ne $false){
            ...
            $easConfigureSilent = Get-Content -Path "$($installerPath)\Variables\Property Files\11.1.2.4.900\Config\Standalone\eas" -Raw
        ...
        if($configEssbase -ne $false){
            ...
            $essbaseServerConfigureSilent = Get-Content -Path "$($installerPath)\Variables\Property Files\11.1.2.4.900\Config\Standalone\essbase" -Raw
        ...
        if($strategic -ne $false){
            ...
            $strategicConfigureSilent = Get-Content -Path "$($installerPath)\Variables\Property Files\11.1.2.4.900\Config\Standalone\strategic" -Raw
        ...
        if($configPlanning -ne $false){
            ...
            $planningConfigureSilent = Get-Content -Path "$($installerPath)\Variables\Property Files\11.1.2.4.900\Config\Standalone\planning" -Raw
        ...
        if($configProfit -ne $false){
            ...
            $profitConfigureSilent = Get-Content -Path "$($installerPath)\Variables\Property Files\11.1.2.4.900\Config\Standalone\profit" -Raw
            $profitConfigureSilent = $ExecutionContext.InvokeCommand.ExpandString($profitConfigureSilent)
        }
        if($configFR -ne $false){
            Write-Host "Adding FR to config file" -ForegroundColor green
            $frConfigureSilent = Get-Content -Path "$($installerPath)\Variables\Property Files\11.1.2.4.900\Config\Standalone\fr" -Raw
            $frConfigureSilent = $ExecutionContext.InvokeCommand.ExpandString($frConfigureSilent)
        }
        if($configFoundation -ne $false){
            Write-Host "Adding Workspace to config file" -ForegroundColor green
            $workspaceConfigureSilent = Get-Content -Path "$($installerPath)\Variables\Property Files\11.1.2.4.900\Config\Standalone\workspace" -Raw 
        ...
        if($configFDM -ne $false){
            ...
            $fdmConfigureSilent = Get-Content -Path "$($installerPath)\Variables\Property Files\11.1.2.4.900\Config\Standalone\fdm" -Raw 
        ...
        if($configHFM -ne $false){
            ...
            $hfmConfigureSilent = Get-Content -Path "$($installerPath)\Variables\Property Files\11.1.2.4.900\Config\Standalone\hfm" -Raw
        ...
        if($firstStage -eq $true){
            ...
            if($remoteDeployment -and $remoteDeployment -eq $true){
                ...
                $headerConfigureSilent = Get-Content -Path "$($installerPath)\Variables\Property Files\11.1.2.4.900\Config\Distributed\remote\header" -Raw
                $headerConfigureSilent = $ExecutionContext.InvokeCommand.ExpandString($headerConfigureSilent)
                ...
                $headerConfigureSilent = Get-Content -Path "$($installerPath)\Variables\Property Files\11.1.2.4.900\Config\Distributed\central\header" -Raw
                $headerConfigureSilent = $ExecutionContext.InvokeCommand.ExpandString($headerConfigureSilent)
            }
            if($configFoundation -ne $false -and $remoteDeployment -eq $true){
                ...
                $fndConfigureSilent = Get-Content -Path "$($installerPath)\Variables\Property Files\11.1.2.4.900\Config\Distributed\Remote\foundation" -Raw
                $fndConfigureSilent = $ExecutionContext.InvokeCommand.ExpandString($fndConfigureSilent)
            ...
            if($configEssbase -ne $false -or $distributedEssbase -eq $true -and $remoteDeployment -eq $false){
                ...
                $apsConfigureSilent =  Get-Content -Path "$($installerPath)\Variables\Property Files\11.1.2.4.900\Config\Distributed\Central\aps" -Raw
                $apsConfigureSilent = $ExecutionContext.InvokeCommand.ExpandString($apsConfigureSilent)
            } 
            if($configEPMA -ne $false){
                Write-Host "Adding Central EPMA to config file" -ForegroundColor green
                $bpmaConfigureSilent = Get-Content -Path "$($installerPath)\Variables\Property Files\11.1.2.4.900\Config\Distributed\Central\epma" -Raw
                $bpmaConfigureSilent = $ExecutionContext.InvokeCommand.ExpandString($bpmaConfigureSilent)
            }
            if($configEssbase -ne $false -or $distributedEssbase -eq $true -and $remoteDeployment -eq $false){
                Write-Host "Adding Central Essbase Studio to config file" -ForegroundColor green
                $bpmsConfigureSilent = Get-Content -Path "$($installerPath)\Variables\Property Files\11.1.2.4.900\Config\Distributed\Central\essbaseStudio" -Raw
                $bpmsConfigureSilent = $ExecutionContext.InvokeCommand.ExpandString($bpmsConfigureSilent)
            }
            if($configCALC -ne $false){
                Write-Host "Adding Central CALC to config file" -ForegroundColor green
                $calcConfigureSilent = Get-Content -Path "$($installerPath)\Variables\Property Files\11.1.2.4.900\Config\Distributed\Central\calc" -Raw
                $calcConfigureSilent = $ExecutionContext.InvokeCommand.ExpandString($calcConfigureSilent)
            }
            if($configEssbase -ne $false -or $distributedEssbase -eq $true -and $remoteDeployment -eq $false){
            ...
            if($configEssbase -ne $false -or $distributedEssbase -eq $true -and $remoteDeployment -eq $false){
                ...
                $essbaseServerConfigureSilent = Get-Content -Path "$($installerPath)\Variables\Property Files\11.1.2.4.900\Config\Distributed\Remote\essbase" -Raw
                $essbaseServerConfigureSilent = $ExecutionContext.InvokeCommand.ExpandString($essbaseServerConfigureSilent)
            }
            if($strategic -eq $true){
                Write-Host "Adding Central Strategic to config file" -ForegroundColor green
                $strategicConfigureSilent = Get-Content -Path "$($installerPath)\Variables\Property Files\11.1.2.4.900\Config\Distributed\Central\strategic" -Raw
                $strategicConfigureSilent = $ExecutionContext.InvokeCommand.ExpandString($strategicConfigureSilent)
            }
            ...
            if($configProfit -ne $false){
                ...
                $profitConfigureSilent = Get-Content -Path "$($installerPath)\Variables\Property Files\11.1.2.4.900\Config\Distributed\Central\profit" -Raw
                $profitConfigureSilent = $ExecutionContext.InvokeCommand.ExpandString($profitConfigureSilent)
            }
            if($configFR -ne $false){
                Write-Host "Adding Central FR to config file" -ForegroundColor green
                $frConfigureSilent = Get-Content -Path "$($installerPath)\Variables\Property Files\11.1.2.4.900\Config\Distributed\Central\fr" -Raw
                $frConfigureSilent = $ExecutionContext.InvokeCommand.ExpandString($frConfigureSilent)
            }
            if($configFoundation -ne $false -and $remoteDeployment -ne $true){
                ...
                $workspaceConfigureSilent = Get-Content -Path "$($installerPath)\Variables\Property Files\11.1.2.4.900\Config\Distributed\Central\workspace" -Raw 
                $workspaceConfigureSilent = $ExecutionContext.InvokeCommand.ExpandString($workspaceConfigureSilent) 
            }
            if($configFDM -ne $false -and $distributedFDM -eq $true){
                ...
                $fdmConfigureSilent = Get-Content -Path "$($installerPath)\Variables\Property Files\11.1.2.4.900\Config\Distributed\Central\fdm" -Raw 
                $fdmConfigureSilent = $ExecutionContext.InvokeCommand.ExpandString($fdmConfigureSilent) 
            }
            ...
            if($configPlanning -ne $false -and $distributedPlanning -eq $true -and $remoteDeployment -ne $true){
                ...
                $planningConfigureSilent = Get-Content -Path "$($installerPath)\Variables\Property Files\11.1.2.4.900\Config\Distributed\Central\planning" -Raw
                $planningConfigureSilent = $ExecutionContext.InvokeCommand.ExpandString($planningConfigureSilent)
                ...
                $planningConfigureSilent = Get-Content -Path "$($installerPath)\Variables\Property Files\11.1.2.4.900\Config\Distributed\Remote\planning" -Raw
                $planningConfigureSilent = $ExecutionContext.InvokeCommand.ExpandString($planningConfigureSilent)
            }
            if($configProfit -ne $false){
            ...
            if($configFDM -ne $false -and $distributedFDM -eq $true -and $remoteDeployment -ne $true){
                ...
                $fdmConfigureSilent = Get-Content -Path "$($installerPath)\Variables\Property Files\11.1.2.4.900\Config\Distributed\Remote\fdm" -Raw 
                $fdmConfigureSilent = $ExecutionContext.InvokeCommand.ExpandString($fdmConfigureSilent) 
            }
            if($configHFM -ne $false -or $distributedHFM -eq $true -and $remoteDeployment -eq $false){
                ...
                $hfmConfigureSilent = Get-Content -Path "$($installerPath)\Variables\Property Files\11.1.2.4.900\Config\Distributed\Central\hfm" -Raw
                $hfmConfigureSilent = $ExecutionContext.InvokeCommand.ExpandString($hfmConfigureSilent)
                ...
                $hfmConfigureSilent = Get-Content -Path "$($installerPath)\Variables\Property Files\11.1.2.4.900\Config\Distributed\Remote\hfm" -Raw
                $hfmConfigureSilent = $ExecutionContext.InvokeCommand.ExpandString($hfmConfigureSilent)
           }
        }
    }
...
#region if any distributed -and remoteDeployment -eq false
    if($distributedHFM -eq $true -or $distributedFDM -eq $true -or $distributedPLN -ew $true -or $distributedESB -eq $true -and $remoteDeployment -eq $false){
    	try {
	     Write-Host "Starting Admin Server for Remote Deployments" -ForegroundColor Cyan
	     Start-Process -FilePath "$($epmInstallPath)\user_projects\domains\EPMSystem\bin\startWebLogic.cmd" -Wait -Verb RunAs -Verbose
	} catch {
